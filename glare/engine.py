...
class Engine(object):
    ...
    def delete(self, context, type_name, artifact_id):
        ...
        if hard_dependency.get_hard_dependencies_children(artifact_id)["graph"]["artifacts"]:
            raise exception.Forbidden
    ...
    def set_hard_dependencies(self, context, source_id, target_id):
        ...
        src_dep_children_graph = hard_dependency.get_hard_dependencies_children(source_id)['graph']['artifacts']
        dependent_source_artifacts = [art["id"] for art in src_dep_children_graph]
        ...
        if source_id == target_id or (target_id in dependent_source_artifacts):
            msg = _("You cannot set hard dependency that will cause a cycle"
                    ...
                    " artifact with uuid:%(source_id)s\n to artifact with uuid: %(target_id)s") %\
