...
class Engine(object):
    ...
    def add_blob_location(self, context, type_name, artifact_id, field_name,
        ...
        # TODO(mfedosin): also implement 'cloned' type here
        if location_type == 'external':
            ...
            if (scheme not in store_api.get_known_schemes() or
                    scheme in store_api.RESTRICTED_URI_SCHEMAS):
                msg = _("Invalid internal location: %s") % location
                raise exception.BadRequest(msg)
