...
"""
Cache driver that uses SQLite to store information about cached blobs
"""
...
sqlite_opts = [
    cfg.StrOpt('blob_cache_sqlite_db', default='cache.db',
               help=_("""
The relative path to sqlite file database that will be used for blob cache
management.
This is a relative path to the sqlite file database that tracks the age and
usage statistics of blob cache. The path is relative to blob cache base
directory, specified by the configuration option ``blob_cache_dir``.
...
class Driver(base.Driver):
    ...
    def initialize_db(self):
        ...
        try:
            ...
            msg = _("Failed to initialize the blob cache database. "
                    "Got error: %s") % e
            LOG.error(msg)
    ...
    def get_hit_count(self, blob_id):
        ...
        with self.get_db() as db:
            cur = db.execute("""SELECT hits FROM cached_blobs
                             WHERE blob_id = ?""",
                             (blob_id,))
            hits = cur.fetchone()[0]
        return hits
    ...
    def get_cached_blobs(self):
        ...
        with self.get_db() as db:
            cur = db.execute("""SELECT
                             blob_id, hits, last_accessed, last_modified, size
                             FROM cached_blobs
                             ORDER BY blob_id""")
    ...
    def delete_all_cached_blobs(self):
        ...
        with self.get_db() as db:
            for path in self.get_cache_files(self.base_dir):
                ...
                deleted += 1
            db.execute("""DELETE FROM cached_blobs""")
            db.commit()
        return deleted
    ...
    def delete_cached_blob(self, blob_id):
        ...
        with self.get_db() as db:
            delete_cached_file(path)
            db.execute("""DELETE FROM cached_blobs WHERE blob_id = ?""",
                       (blob_id, ))
            db.commit()
    ...
    def get_least_recently_accessed(self):
        ...
        """
        with self.get_db() as db:
            cur = db.execute("""SELECT blob_id FROM cached_blobs
                             ORDER BY last_accessed LIMIT 1""")
            try:
                blob_id = cur.fetchone()[0]
            except TypeError:
                # There are no more cached blobs
                return None
    ...
    def open_for_write(self, blob_id):
        ...
        def commit():
            with self.get_db() as db:
                ...
                db.execute("""INSERT INTO cached_blobs
                           (blob_id, last_accessed, last_modified, hits, size)
                           VALUES (?, ?, ?, 0, ?)""",
                           (blob_id, now, now, filesize))
                db.commit()
    ...
    def open_for_read(self, blob_id):
        ...
        now = time.time()
        with self.get_db() as db:
            db.execute("""UPDATE cached_blobs
