...
class TestDynamicQuotas(base.TestArtifact):
    ...
    def test_count_artifact_number(self):
        ...
        # define several quotas for user1
        body = {'quota_name': 'max_artifact_number',
                'type_name': 'images',
                'quota_value': 3}
        q = self.post(url=url, data=body)
        self.assertEqual(user1_tenant_id, q.pop('project_id'))
        self.assertIsNotNone(q.pop('id'))
        ...
        self.assertEqual(user1_tenant_id, q.pop('project_id'))
        self.assertIsNotNone(q.pop('id'))
        self.assertIsNone(q.pop('type_name'))
        self.assertEqual(body, q)
        ...
        body = {'quota_name': 'max_artifact_number',
                'quota_value': 10}
        q = self.post(url=url, data=body)
        self.assertEqual(admin_tenant_id, q.pop('project_id'))
        self.assertIsNotNone(q.pop('id'))
        self.assertIsNone(q.pop('type_name'))
        self.assertEqual(body, q)
    ...
    def test_calculate_uploaded_data(self):
        ...
        url = '/quotas/' + user1_tenant_id
        # define several quotas for user1
        body = {'quota_name': 'max_uploaded_data',
                'type_name': 'images',
                'quota_value': 1500}
        q = self.post(url=url, data=body)
        self.assertEqual(user1_tenant_id, q.pop('project_id'))
