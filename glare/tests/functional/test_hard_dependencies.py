...
class TestHardDependencies(base.TestArtifact):
    ...
    def assert_hard_dependencies(self, art_list,
        ...
        for i in range(art_num):
            if expected_of_hard_dep_nodes_list[i] is None:
                self.get(url="/dependencies/%s" % art_list[i]["id"], status=404)
            else:
                graph = self.get(url="/dependencies/%s" % art_list[i]["id"])["graph"]
    ...
    def test_hard_dependencies_basic(self):
        ...
        self.assert_hard_dependencies(art_list, [4, 3, 0, 1, 0],[4, 3, 0, 1, 0])
        ...
        self.assert_hard_dependencies(art_list, [None, 2, 0, 1, 0],
                                      [None, 2, 0, 1, 0])