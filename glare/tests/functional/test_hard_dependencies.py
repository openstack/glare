...
class TestHardDependencies(base.TestArtifact):
    ...
    def test_hard_dependencies_basic(self):
        ...
        deps1 = self.get(url="/dependencies/%s" %
                             art_list[1]["id"])["graph"]
        ...
        # create art from user2
        self.set_user('user2')
        # self.set_user('admin')
        user2_art_list = [self.create_artifact({'name': 'user2_name%s' % i})
                          for i in range(5)]

        url = '/dependencies'
        # Set hard dependencies between artifacts
        values = {"source_id": user2_art_list[0]['id'],
                  "target_id": user2_art_list[1]['id']}
        self.post(url=url, data=values, status=204)

        # Negative - set H.D to arts from different tenants
        values = {"source_id": user2_art_list[1]['id'],
                  "target_id": art_list[0]['id']}
        # resource from other tenant could not be found (user 2)
        self.post(url=url, data=values, status=404)  # Todo(Mike)-review

        # Deletions #

        #  Negative (current user: user2 tenant)
        #     1. Delete H.D
        values = {"source_id": art_list[0]['id'],
                  "target_id": art_list[1]['id']}
        self.delete(url="/dependencies/%(source_id)s/%(target_id)s" % values,
        ...
        self.delete('/sample_artifact/%s' % art_list[0]['id'], status=404)
