...
class TestQuotaFunctions(base.BaseTestArtifactAPI):
    ...
    def test_count_artifact_number(self):
        ...
        for type_name in amount:
            num = db_api.count_artifact_number(self.req.context, type_name)
            self.assertEqual(amount[type_name], num)

        # count the whole amount of artifacts
        self.assertEqual(17, db_api.count_artifact_number(self.req.context))
    ...
    def test_calculate_uploaded_data(self):
        ...
        self.assertEqual(10, db_api.calculate_uploaded_data(self.req.context))
        ...
        self.assertEqual(115, db_api.calculate_uploaded_data(self.req.context))
        ...
        self.assertEqual(215, db_api.calculate_uploaded_data(self.req.context))
        ...
        self.assertEqual(365, db_api.calculate_uploaded_data(self.req.context))
        # 215 bytes for sample_artifact
        self.assertEqual(215, db_api.calculate_uploaded_data(
            self.req.context, 'sample_artifact'))
        # 150 bytes for images
        self.assertEqual(150, db_api.calculate_uploaded_data(
            self.req.context, 'images'))
        ...
        self.assertEqual(365, db_api.calculate_uploaded_data(self.req.context))
        # user2 has 1000
        self.assertEqual(1000, db_api.calculate_uploaded_data(req.context))
