...
#
# There is an ongoing work to extract similar code to oslo incubator. Once
# it is
...
class BaseWalkMigrationTestCase(object):
    ...
    ALEMBIC_CONFIG = alembic_config.Config(
        os.path.join(os.path.dirname(glare.db.migration.__file__),
                     'alembic.ini')
    )
    ...
    def _configure(self, engine):
        ...
        should use oslo_config and oslo_db.sqlalchemy.session with
