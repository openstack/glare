...
class TestHardDependeciesFunctions(base.BaseTestArtifactAPI):
    ...
    def test_get_and_set_hard_dependencies(self):
        ...
        self.assertEqual({'graph': {'artifacts': [], 'dependencies': []}},
                         api.get_hard_dependencies(art['id'], self.session))
        ...
        hard_dep_list_graph_0 = api.get_hard_dependencies(art_list[0]['id'],
                                                    self.session)["graph"]
        ...
        hard_dep_list_graph_1 = api.get_hard_dependencies(art_list[1]['id'],
                                                    self.session)["graph"]
        ...
        api.delete_hard_dependencies(art_list[1]['id'],
                                     art_list[2]['id'], self.session )
        # Get H.D for art1
        hard_dep_list_graph_1 = api.get_hard_dependencies(
            art_list[1]['id'], self.session)["graph"]
