...
class TestQuotaFunctions(base.BaseTestArtifactAPI):
    ...
    def test_count_artifact_number(self):
        ...
        self.config(max_artifact_count=10)
        # 3 images, 5 heat templates, 10 murano packages
        self.config(max_artifact_count=3, group='images')
        self.config(max_artifact_count=5, group='heat_templates')
        self.config(max_artifact_count=10, group='murano_packages')
        ...
        for i in range(3):
            self.controller.create(user1_req, 'images', {'name': 'img%d' % i})

        # creation of another image fails
