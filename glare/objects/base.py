...
class BaseArtifact(base.VersionedObject):
    ...
    def download_blob(cls, context, af, field_name):
        ...
        meta = {'size': blob.get('size'), 'checksum': blob.get('checksum'),
    ...
    def add_blob_location(cls, context, af, field_name, location, blob_meta):
        ...
        try:
            for k in ('size', 'checksum', 'content_type'):
                if k in blob_meta:
                    blob[k] = blob_meta[k]
                    if k == 'size':
                        blob[k] = int(blob[k])
        except ValueError as e:
            msg = (_("Incorrect blob metadata %(meta)s. Got error: %(error)s"),
                   {'meta': str(blob_meta), 'error': str(e)})
            raise exception.BadRequest(msg)
    ...
    def add_blob_dict_location(cls, context, af, field_name,
        ...
        try:
            ...
            msg = (_("Incorrect blob metadata %(meta)s. "
                     "Got error: %(error)s"),
                   {'meta': str(blob_meta), 'error': str(e)})
            raise exception.BadRequest(msg)
