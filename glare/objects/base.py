...
class BaseArtifact(base.VersionedObject, base.VersionedObjectDictCompat, dict):
    ...
    def create(cls, context, values):
        ...
        af = cls._init_artifact(context, init_values)
        # enable additional checks to validate user input and apply
        # other values specified by user
        af._user_input = True
        for name, value in values.items():
            if name not in af.fields:
                msg = _("%s field does not exist") % name
    ...
    def update_artifact(cls, context, af_id, values):
        ...
        :param context: user context
        :param af_id: id of artifact to update
        ...
        LOG.info("Parameters validation for artifact %(artifact)s "
                 ...
                 {'artifact': af_id, 'request': context.request_id})
        updated_af = cls.db_api.update(context, af_id, values)
    ...
    def delete(cls, context, af):
        ...
        cls.db_api.update(context, af.id, {'status': 'deleted'})
