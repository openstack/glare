...
def _apply_user_filters(query, basic_conds, tag_conds, prop_conds):
    ...
    if len(or_queries) != 0:
        query = query.join(models.ArtifactProperty, aliased=True).join(models.ArtifactTag, aliased=True).filter(
            or_(*or_queries))
...
def _do_query_filters(filters):
    ...
    for field_name, key_name, op, field_type, value, query_combiner in filters:
        if field_name == 'tags':
            ...
            for tag in tags:
                tag_conds[query_combiner].append([models.ArtifactTag.value == tag])
        elif field_name == 'tags-any':
            tags = utils.split_filter_value_for_quotes(value)
            tag_conds[query_combiner].append([models.ArtifactTag.value.in_(tags)])
