...
def get_hard_dependencies_children(artifact_id, session):
    ...
    artifacts = [{"id": art_id, "values": art_id} for art_id in nodes if
                 art_id != artifact_id]  # todo: (Idan) the value section
...
def delete_hard_dependencies(art_source_id, art_target_id, session):
    ...
    with session.begin():
        ...
        for row in orig_query:
            rows_to_del_set = \
                recursive_delete_hard_dependencies(session,
                                                   art_source_id,
                                                   row.artifact_target,
                                                   rows_to_del_set)
        orig_query.delete()
...
@retry(retry_on_exception=_retry_on_deadlock, wait_fixed=500,
       stop_max_attempt_number=50)
def recursive_delete_hard_dependencies(session, art_origin_id,
