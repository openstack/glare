...
@retry(retry_on_exception=_retry_on_deadlock, wait_fixed=500,
       stop_max_attempt_number=50)
def get_hard_dependencies(context, artifact_id, session):
    ...
    # TODO(Idan) extract basic values of the artifact, from artifact it
    ...
    response = {"graph": {"artifacts": artifacts,
...
@retry(retry_on_exception=_retry_on_deadlock, wait_fixed=500,
       stop_max_attempt_number=50)
def get_hard_dependencies_children(context, artifact_id, session):
    ...
    artifacts = [{"id": art_id, "values": get(context, art_id, session)}
                 for art_id in nodes if art_id != artifact_id]
...
@retry(retry_on_exception=_retry_on_deadlock, wait_fixed=500,
       stop_max_attempt_number=50)
def set_hard_dependencies(context, source, target, session):
    ...
    with session.begin():
        ...
        constraint1 = models.ArtifactDependencies.artifact_source == source
        constraint2 = models.ArtifactDependencies.artifact_origin == source
        constraint3 = models.ArtifactDependencies.artifact_target == target
        existed_h_d_con = and_(constraint1, constraint2, constraint3)
        q = session.query(models.ArtifactDependencies).filter(existed_h_d_con)
        if len(q.all()) > 0:
            return
        ...
        for query_row in dependencies:
            ...
            #  3->5, 1->2, 2->3
            recursive_set(context, query_row, session)
...
@retry(retry_on_exception=_retry_on_deadlock, wait_fixed=500,
       stop_max_attempt_number=50)
def recursive_set(context, query_row, session):
    cond = models.ArtifactDependencies.artifact_target ==\
        query_row.artifact_origin
...
@retry(retry_on_exception=_retry_on_deadlock, wait_fixed=500,
       stop_max_attempt_number=50)
def delete_hard_dependencies(context, art_source_id, art_target_id, session):
    ...
    with session.begin():
        ...
        cond1 = models.ArtifactDependencies.artifact_source == art_source_id
        cond2 = models.ArtifactDependencies.artifact_target == art_target_id
        cond = and_(cond1, cond2)

        # delete the row #
        orig_query = session.query(models.ArtifactDependencies).filter(cond)
        rows_to_del_set = set()

        # Recursive #
        # get all the rows that are no longer relevant due to the H.D deletion
        for row in orig_query:
            rows_to_del_set = \
                recursive_delete_hard_dependencies(context, session,
                                                   art_source_id,
                                                   row.artifact_target,
                                                   rows_to_del_set)
        orig_query.delete()
...
@retry(retry_on_exception=_retry_on_deadlock, wait_fixed=500,
       stop_max_attempt_number=50)
def recursive_delete_hard_dependencies(context, session, art_origin_id,
                                       art_target_id, rows_to_del_set):
    ...
    rows_to_del = session.query(models.ArtifactDependencies).filter(cond)
    # rows_to_del_set.add(rows_to_del)
    for row in rows_to_del:
        rows_to_del_set = recursive_delete_hard_dependencies(
            context, session, row.artifact_origin,
            row.artifact_target, rows_to_del_set)
...
@retry(retry_on_exception=_retry_on_deadlock, wait_fixed=500,
       stop_max_attempt_number=50)
def delete_hard_dependencies_for_art_deletion(context, art_to_be_deleted_id,
                                              session):
    ...
    with session.begin():
        cond = models.ArtifactDependencies.artifact_origin == \
            art_to_be_deleted_id
        session.query(models.ArtifactDependencies).filter(cond).delete()
